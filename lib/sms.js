...
sms.prototype.send = function(options, callback) {
  ...
  this.request.post({
    ...
    try {
      ...
      // check the message-count and attach proper status text with meaning
      var messageCount = parseInt(res['message-count']);
      while(messageCount--) {
        var message = res['messages'][messageCount];
        var status = parseInt(message.status);
        message.statusText = Codes[status].status;
        message.meaning = Codes[status].meaning;
      }
